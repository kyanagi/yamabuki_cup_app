...
RSpec.describe MatchRule::Round3Hayabo2 do
  ...
  describe "#process_question_closing" do
    ...
    context "正解で18点になったとき" do
      ...
      it "勝ち抜けになること" do
        initial_scores[0].update!(points: 14)
        match_rule.process_question_closing(question_closing, question_player_results)
        scores = match_rule.instance_variable_get(:@scores)
        expect(scores[0].points).to eq 18
    ...
    context "正解で19点以上になったとき" do
      ...
      it "勝ち抜けになること" do
        initial_scores[0].update!(points: 16)
        match_rule.process_question_closing(question_closing, question_player_results)
        scores = match_rule.instance_variable_get(:@scores)
        expect(scores[0].points).to eq 20
        expect(scores[0].status).to eq "win"
      end
    end
    ...
    context "複数人が同時に18点以上になったとき" do
      ...
      it "点数の高い順に上位になること" do
        initial_scores[0].update!(points: 16)
        initial_scores[1].update!(points: 16)
        match_rule.process_question_closing(question_closing, question_player_results)
        scores = match_rule.instance_variable_get(:@scores)
        expect(scores[0].points).to eq 18
        expect(scores[0].status).to eq "win"
        expect(scores[0].rank).to eq 2
        expect(scores[1].points).to eq 19
    ...
    context "複数人が同時に18点以上になり、点数が同じとき" do
      ...
      it "座席の昇順に勝ち抜けになること" do
        ...
        expect(scores[1].points).to eq 18
        expect(scores[1].status).to eq "win"
    ...
    describe "複数人の処理" do
      ...
      it "全員の得点が正しく計算されること" do
        match_rule.process_question_closing(question_closing, question_player_results)
        scores = match_rule.instance_variable_get(:@scores)
        expect(scores.map(&:points)).to eq [2, 0, 3, 0, 2, 2, 0, 2]
      end
    end
  end
