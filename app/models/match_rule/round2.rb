module MatchRule
  class Round2 < Base
    ...
    def judge_on_quiz_completed
      num_left_winners = NUM_WINNERS - @matchings.count(&:status_win?)
    ...
    def process_correct(matching)
      matching.points += 1
      if matching.points >= POINTS_TO_WIN
        mark_as_winner(matching)
      end
    ...
    def process_wrong(matching)
      matching.misses += 1
      if matching.misses >= MISSES_TO_LOSE
    ...
    def fill_vacant_buttons_with_waiting_players
      n = NUM_BUTTONS - @matchings.count(&:status_playing?)
      return if n <= 0
    ...
    def num_left_players_is_equal_to_num_left_winners?
      @matchings.count(&:status_lose?) == NUM_SEATS - NUM_WINNERS
    end
